...
<script lang="ts">
  ...
  import { AddEquation, Color, CustomBlending, Group, LessEqualDepth, OneFactor } from 'three';

  export const ref = new Group()

  const gltf = useGltf('/spaceship-4k.glb')
  const map = useTexture('/energy-beam-opacity.png')

  const tail = 2000;

  gltf.then((model) => {
    function alphaFix(material: { transparent: boolean; alphaToCoverage: boolean; depthFunc: number; depthTest: boolean; depthWrite: boolean; }) {
  ...
  {:then gltf}
    ...
    >
      ...
      {#await map then mapValue}
        ...
        <T.Mesh
          position={[742, -64, -0.8*tail]}
          rotation.x={Math.PI * 0.5}
        >
          <T.CylinderGeometry args={[64, 0, tail, 10, 32]} openEnded={true} radialSegments={32}  />
          <T.MeshBasicMaterial
            color={Color.NAMES.orangered}
            alphaMap={mapValue}
            transparent
